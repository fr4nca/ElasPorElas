    ...
    query(`SELECT email FROM mulher WHERE email='${email}'`, (err, result) => {
      if (result[0] === undefined) {
        if (req.body.link_rede_social !== "") {
          const qry = `INSERT INTO mulher(CPF, nome, DDD_telefone, num_telefone, email, link_rede_social, senha) VALUES ('${CPF}', '${nome}', ${DDD_telefone}, ${num_telefone}, '${email}', '${link_rede_social}', '${hash}')`;
          query(qry, err => {
            if (err) {
